...
export default function App() {
    ...
    if (isLoggedIn) {
      ...
      const handleUserJoinedCall = async (data: { roomId: string; username: string; socketId: string }) => {
        ...
        if (activeCall && activeCall.isGroup && activeCall.roomId === data.roomId) {
          const pc = setupPeerConnection(data.username, data.socketId);
          if (localStream.current) {
            addTrackToPc(pc, localStream.current);
          }
          // Negotiation will be triggered by pc.addTrack via onnegotiationneeded
        }
      };
      ...
      const handleIceCandidate = async (data: { socket: string; candidate: any; from: string }) => {
        if (data.candidate) {
          console.log(`Received ICE candidate from ${data.from} (${data.socket})`);
          const pc = peerConnections.current.get(data.from);
          if (pc) {
            try {
            ...
            console.warn(`PeerConnection not found for ${data.from}`);
          }
        }
      };
  ...
  const setupPeerConnection = (targetId: string, remoteSocketId?: string) => {
    ...
    // Pre-create transceivers to lock m-line order (audio then video)
    ...
    pc.onnegotiationneeded = async () => {
      try {
        if (pc.signalingState !== 'stable') {
          console.log(`Skipping negotiation for ${targetId} - signalingState is ${pc.signalingState}`);
          return;
        }
        ...
        const to = remoteSocketId || activeCallRef.current?.remoteSocketId || targetId;
        socket.emit('call-user', { to, offer, from: usernameRef.current, type: activeCallRef.current?.type || 'audio' });
      } catch (e) {
  ...
  const startCall = async (targetName: string, type: 'audio' | 'video') => {
    ...
    try {
      ...
      setActiveCall({ 
        ...
        remoteSocketId: Array.from(socketToUsername.entries()).find(([_, name]) => name === targetName)?.[0]
      });
      setIsCallMinimized(false);
      ...
      if (activeRoom?.startsWith('group-')) {
        ...
        // We can use targetName directly as the ID, the server will resolve it to a socket ID
  ...
  const joinRoomCall = async (roomId: string) => {
    try {
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          audio: selectedAudioId !== 'default' ? { deviceId: { ideal: selectedAudioId } } : true,
          video: activeRoomCall?.type === 'video' ? (selectedVideoId !== 'default' ? { deviceId: { ideal: selectedVideoId } } : true) : false
        });
      } catch (e) {
        console.warn("Failed with selected devices, falling back to default", e);
        stream = await navigator.mediaDevices.getUserMedia({
  ...
  const answerCall = async () => {
    ...
    try {
      ...
      if (data.offer) {
        ...
        addTrackToPc(pc, stream);
        // onnegotiationneeded will handle creating the offer if the state is stable
      }
    } catch (err) {
                ...
                <AnimatePresence mode="popLayout">
                  {recentChats.map((chat, i) => (
                    ...
                    >
                        <div className={cn(
                          "relative",
                          userList.find(u => u.id === chat.username)?.is_premium === 1 && "p-[2px] rounded-full bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 animate-gradient-xy"
                        )}>
                          <img src={chat.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${chat.username}`} alt={chat.username} className="w-12 h-12 rounded-full bg-white/5 border-2 border-[#141414]" referrerPolicy="no-referrer" />
                      ...
                      <div className="flex-1 text-left truncate">
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-1.5 truncate">
                            ...
                            {userList.find(u => u.id === chat.username)?.is_premium === 1 && (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <motion.h2 layoutId="header-title" className="font-bold text-lg tracking-tight truncate flex items-center gap-2">
                        ...
                        {!activeRoom?.startsWith('group-') && userList.find(u => u.id === activeRoomName)?.is_premium === 1 && (
                        ...
                        {msg.sender !== username && (
                          <div className="flex items-center gap-1">
                            <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                              {userList.find(u => u.id === msg.sender)?.name || msg.sender}
                            </span>
                            {userList.find(u => u.id === msg.sender)?.is_premium === 1 && (
            ...
            >
                ...
                {activeCall.isGroup && activeRoomCall ? (
                  <div className="w-full h-full flex flex-col">
                    <div className="p-6 text-center">
                      <h2 className="text-2xl font-bold text-white mb-1">{activeRoomName}</h2>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {activeRoomCall.participants.map((participant) => {
                          ...
                          const stream = participant === username ? localStream.current : activeCall.remoteStreams?.[participant];
                              ...
                              <>
                                <div className={cn(
                                  ...
                                  <div className="w-full h-full rounded-full bg-[#1c242c] flex items-center justify-center overflow-hidden border-2 border-[#1c242c]">
                                    <img 
                                      src={participant === username ? (profile?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${participant}`) : (userList.find(u => u.id === participant)?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${participant}`)} 
                                      alt={participantName} 
                                      className="w-full h-full object-cover"
                                      referrerPolicy="no-referrer"
                                    />
                  ...
                  <div className="flex flex-col items-center justify-center text-center space-y-8">
                        ...
                        <div className="w-full h-full rounded-full bg-[#1c242c] flex items-center justify-center overflow-hidden border-4 border-[#1c242c]">
                          <img 
                            src={(() => {
                              ...
                              return userList.find(u => u.id === otherUser)?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${otherUser}`;
                            })()} 
                            alt="Avatar" 
                            className="w-full h-full object-cover"
                            referrerPolicy="no-referrer"
                    ...
                    <div className="space-y-3">
                      <h2 className="text-2xl font-medium text-white tracking-wide">
                        {(() => {
                          const otherUser = activeCall.isIncoming ? activeCall.from : activeCall.to;
                          return userList.find(u => u.id === otherUser)?.name || otherUser;
                        })()}
                      </h2>
              ...
              <div className="hidden">
                {activeCall.isGroup && activeCall.remoteStreams ? (
                  Object.entries(activeCall.remoteStreams).map(([participantId, stream]) => (
                    <audio 
                      key={participantId}
                      autoPlay 
