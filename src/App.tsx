...
export default function App() {
      ...
      if (activeCall && localStream.current) {
        ...
        try {
          const newStream = await navigator.mediaDevices.getUserMedia({
            audio: selectedAudioId !== 'default' ? { deviceId: { exact: selectedAudioId } } : true,
            video: isVideoOn ? (selectedVideoId !== 'default' ? { deviceId: { exact: selectedVideoId } } : true) : false
          });
  ...
  const addTrackToPc = (pc: RTCPeerConnection, stream: MediaStream) => {
    ...
    // Always add audio first, then video to maintain m-line order in SDP
  ...
  const setupPeerConnection = (targetId: string, remoteSocketId?: string) => {
    ...
    if (pc) {
      // If we have a new socket ID for the same target, we should update our signaling logic
      // but the PC itself is still valid.
      return pc;
    }
    ...
    pc = new RTCPeerConnection({
      iceServers: [
        // STUN servers (for finding public IP)
        ...
        // Free TURN servers from OpenRelay (for relaying traffic when P2P fails)
        // These are public community servers, great for testing and small apps
        {
    ...
    try {
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          audio: selectedAudioId !== 'default' ? { deviceId: { exact: selectedAudioId } } : true,
          video: type === 'video' ? (selectedVideoId !== 'default' ? { deviceId: { exact: selectedVideoId } } : true) : false
        });
      } catch (e) {
  ...
  const joinRoomCall = async (roomId: string) => {
    try {
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          audio: selectedAudioId !== 'default' ? { deviceId: { exact: selectedAudioId } } : true,
          video: activeRoomCall?.type === 'video' ? (selectedVideoId !== 'default' ? { deviceId: { exact: selectedVideoId } } : true) : false
        });
      } catch (e) {
        console.warn("Failed with selected devices, falling back to default", e);
        stream = await navigator.mediaDevices.getUserMedia({
  ...
  const answerCall = async () => {
    ...
    try {
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          audio: selectedAudioId !== 'default' ? { deviceId: { exact: selectedAudioId } } : true,
          video: data.type === 'video' ? (selectedVideoId !== 'default' ? { deviceId: { exact: selectedVideoId } } : true) : false
        });
      } catch (e) {
        console.warn("Failed with selected devices, falling back to default", e);
        stream = await navigator.mediaDevices.getUserMedia({
