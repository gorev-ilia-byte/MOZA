...
import { Message, User, Room, Profile, UserSettings, Poll, Sticker } from './types';
...
const PollMessage = ({ pollId, polls, votePoll, username }: { pollId: string, polls: { [id: string]: Poll }, votePoll: (id: string, index: number) => void, username: string }) => {
  const poll = polls[pollId];
  if (!poll) return <div className="p-4 bg-zinc-900/50 rounded-2xl border border-white/5 italic text-zinc-500">Poll loading...</div>;

  const totalVotes = Object.values(poll.votes).reduce((acc, curr) => acc + curr.length, 0);
  const userVotedIndex = Object.entries(poll.votes).find(([_, users]) => users.includes(username))?.[0];

  return (
    <motion.div 
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
      className="p-4 bg-zinc-900/80 backdrop-blur-md rounded-2xl border border-white/10 shadow-xl w-full max-w-sm space-y-4"
    >
      <div className="flex items-center gap-2 mb-2">
        <BarChart2 className="w-4 h-4 text-emerald-500" />
        <span className="text-xs font-bold text-emerald-500 uppercase tracking-widest">Poll</span>
      </div>
      <h4 className="font-bold text-white text-lg leading-tight">{poll.question}</h4>
      <div className="space-y-2">
        {poll.options.map((option, index) => {
          const optionVotes = poll.votes[index]?.length || 0;
          const percentage = totalVotes > 0 ? (optionVotes / totalVotes) * 100 : 0;
          const isVoted = userVotedIndex === String(index);

          return (
            <button
              key={index}
              onClick={() => votePoll(pollId, index)}
              className="w-full relative group overflow-hidden rounded-xl border border-white/5 hover:border-emerald-500/30 transition-all active:scale-[0.98]"
            >
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${percentage}%` }}
                className={cn(
                  "absolute inset-0 bg-emerald-500/10 transition-all duration-1000",
                  isVoted && "bg-emerald-500/20"
                )}
              />
              <div className="relative p-3 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    isVoted ? "border-emerald-500 bg-emerald-500" : "border-zinc-700 group-hover:border-zinc-500"
                  )}>
                    {isVoted && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                  </div>
                  <span className={cn("text-sm font-medium transition-colors", isVoted ? "text-white" : "text-zinc-400 group-hover:text-zinc-200")}>
                    {option}
                  </span>
                </div>
                {totalVotes > 0 && (
                  <span className="text-xs font-bold text-zinc-500">{Math.round(percentage)}%</span>
                )}
              </div>
            </button>
          );
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/5">
        <span className="text-[10px] text-zinc-500 font-medium">{totalVotes} votes</span>
        <span className="text-[10px] text-zinc-600 italic">Created by {poll.creator}</span>
      </div>
    </motion.div>
  );
};

const StickerPicker = ({ stickers, sendSticker, setShowStickerPicker }: { stickers: Sticker[], sendSticker: (s: Sticker) => void, setShowStickerPicker: (b: boolean) => void }) => {
  const defaultStickers = [
    "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/3o7TKMGpxS6S5F8f0Q/giphy.gif",
    "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/l41lTfuxX5Wp99v5S/giphy.gif",
    "https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/3o7TKVUn7iM8FMEU24/giphy.gif",
  ];
  ...
  return (
    <motion.div 
      ...
      className="absolute bottom-full right-0 mb-4 w-72 h-96 bg-zinc-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col z-50"
    >
      <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
        <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Stickers</h4>
        <button onClick={() => setShowStickerPicker(false)} className="p-1 hover:bg-white/10 rounded-lg transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-4 grid grid-cols-3 gap-3 custom-scrollbar">
        {defaultStickers.map((url, i) => (
          <button 
            key={i} 
            onClick={() => sendSticker({ id: `default-${i}`, url, packName: 'Classic', creator: 'system' })}
            className="aspect-square rounded-xl overflow-hidden hover:scale-110 transition-transform active:scale-95"
          >
        ...
        {stickers.map((sticker) => (
          <button 
            ...
            className="aspect-square rounded-xl overflow-hidden hover:scale-110 transition-transform active:scale-95"
          >
            <img src={sticker.url} alt="sticker" className="w-full h-full object-contain" referrerPolicy="no-referrer" />
          </button>
        ))}
      </div>
    </motion.div>
...
export default function App() {
  ...
  const [activeSettingsTab, setActiveSettingsTab] = useState<'main' | 'notifications' | 'privacy' | 'data' | 'devices' | 'language' | 'faq' | 'ask' | 'policy' | 'media' | 'premium' | 'appearance'>('main');
  const [showPremiumModal, setShowPremiumModal] = useState(false);
  const [showPollModal, setShowPollModal] = useState(false);
  const [showStickerPicker, setShowStickerPicker] = useState(false);
  const [polls, setPolls] = useState<{ [id: string]: Poll }>({});
  ...
  const handleStoryUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (!isPremium) {
        alert("Only Premium users can post stories!");
        return;
      }
      const limit = 50 * 1024 * 1024; // 50MB for stories
      if (file.size > limit) {
      ...
      reader.onloadend = () => {
        ...
        socket.emit("create-story", { username, media_url: base64String, media_type: type });
      };
  ...
  useEffect(() => {
    ...
    if ('Notification' in window && Notification.permission === 'default') {
      // We don't automatically prompt on mount to avoid being annoying, 
      // but we could if we wanted to.
      console.log("Notification permission is default. User can enable it in settings.");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    socket.on("poll-updated", (data: { pollId: string; votes: { [key: number]: string[] } }) => {
      setPolls(prev => {
        if (!prev[data.pollId]) return prev;
        return {
          ...prev,
          [data.pollId]: { ...prev[data.pollId], votes: data.votes }
        };
      });
    });

    socket.on("polls-list", (pollsList: Poll[]) => {
      const pollsMap: { [id: string]: Poll } = {};
      pollsList.forEach(p => pollsMap[p.id] = p);
      setPolls(pollsMap);
    });

    socket.on("stickers-list", (stickersList: Sticker[]) => {
      setStickers(stickersList);
    });

    socket.emit("get-stickers");

    return () => {
      socket.off("poll-created");
      socket.off("poll-updated");
      socket.off("polls-list");
      socket.off("stickers-list");
    };
  }, []);

  useEffect(() => {
    if (activeRoom) {
      socket.emit("get-polls", activeRoom);
    }
  }, [activeRoom]);
  ...
  const votePoll = (pollId: string, optionIndex: number) => {
    socket.emit("vote-poll", { pollId, username, optionIndex });
  };
  ...
  useEffect(() => {
    const updateActiveDevices = async () => {
      if (activeCall && localStream.current) {
        ...
        try {
          // Stop old tracks first to avoid "Device in use" errors in some browsers
          ...
          // If it fails, we might be left with no tracks. 
          // In a real app we'd try to restore the old ones, but here we'll just log.
        }
      }
    };
    updateActiveDevices();
  ...
  useEffect(() => {
    if (activeCall?.remoteStream) {
      console.log("Updating remote stream elements", activeCall.remoteStream.getTracks().map(t => t.kind));
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      const handleCallMade = async (data: { offer: any; socket: string; from: string; type: 'audio' | 'video' }) => {
        ...
        if (currentCall && data.offer) {
          console.log("Handling re-negotiation offer");
  ...
  const deleteMessage = (messageId: string | number) => {
    ...
    if (msg) {
      socket.emit('delete-message', { messageId, room: activeRoom, sender: msg.sender });
    }
  };
    ...
    stream.getTracks().forEach(track => {
      ...
      if (sender) {
        ...
        try {
          ...
          console.warn(`Error adding ${track.kind} track to PC:`, e);
        }
      }
    });
  };
  ...
  const triggerNegotiation = async () => {
    if (peerConnection.current && activeCallRef.current) {
      try {
        console.log("Triggering re-negotiation");
        const offer = await peerConnection.current.createOffer();
        await peerConnection.current.setLocalDescription(offer);
        const target = activeCallRef.current.remoteSocketId || 
                     (activeCallRef.current.isIncoming ? activeCallRef.current.from : activeCallRef.current.to);
        if (!target) {
          console.warn("No target for negotiation");
          return;
        }
  ...
  const setupPeerConnection = (targetId: string, remoteSocketId?: string) => {
    ...
    pc.ontrack = (event) => {
      ...
      setActiveCall(prev => {
        if (!prev) return null;
        const remoteStream = new MediaStream(prev.remoteStream?.getTracks() || []);
        if (!remoteStream.getTracks().find(t => t.id === event.track.id)) {
          remoteStream.addTrack(event.track);
        }
        const hasVideo = remoteStream.getVideoTracks().length > 0 || prev.type === 'video';
        return { ...prev, type: hasVideo ? 'video' : prev.type, remoteStream };
      });
    };
  ...
  const startCall = async (targetName: string, type: 'audio' | 'video') => {
    if (isMediaRequesting.current) return;
  ...
  const answerCall = async () => {
    ...
    // Stop any existing stream before answering a new call
    stopStream(localStream.current);
    localStream.current = null;
  ...
  const requestNotificationPermission = async () => {
    ...
    try {
      ...
      if (permission === 'granted') {
        showNotification(userSettings?.language === 'Russian' ? 'Уведомления включены!' : 'Notifications Enabled!', {
          body: userSettings?.language === 'Russian' ? 'Теперь вы будете получать уведомления о сообщениях и звонках.' : 'You will now receive notifications for messages and calls.',
          icon: '/favicon.ico'
        });
      }
    } catch (error) {
  ...
  const testNotification = () => {
    showNotification(userSettings?.language === 'Russian' ? 'Тестовое уведомление' : 'Test Notification', {
      body: userSettings?.language === 'Russian' ? 'Это проверка того, как выглядят уведомления в фоне.' : 'This is a test of how background notifications look.',
      icon: profiles[username]?.avatar_url || DEFAULT_AVATAR
    });
  };
  ...
  const showNotification = (title: string, options?: NotificationOptions) => {
    ...
    // Show if permission is granted AND (tab is hidden OR user explicitly requested a test)
  ...
  if (!isLoggedIn) {
    return (
      <div className={cn(
        "min-h-screen flex items-center justify-center p-4 transition-colors duration-700 liquid-bg overflow-hidden",
        ...
        <motion.div 
          ...
          className={cn(
            "w-full max-w-[450px] glass-card rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden z-10 border border-white/10",
        ...
        >
          ...
          >
            <div className="relative">
              ...
              <div className="w-20 h-20 bg-emerald-500/20 rounded-3xl flex items-center justify-center border border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.2)] relative z-10">
          ...
          >
            <h1 className="text-3xl font-bold text-white mb-2 tracking-tight">MOZA</h1>
            <div className="flex items-center justify-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <p className="text-zinc-400 text-sm font-medium">
                {isDesktop ? "Secure Workstation" : "Mobile Client"}
              </p>
            </div>
          </motion.div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <AnimatePresence mode="wait">
              {authError && (
                <motion.div 
                  ...
                  className="text-red-400 text-sm text-center bg-red-500/10 border border-red-500/20 py-2 rounded-xl"
                >
                  {authError}
                </motion.div>
              )}
            ...
            <div className="space-y-4">
                ...
                {isRegistering && (
                  ...
                  >
                    <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-4 mb-2">
                      ...
                      <ul className="text-[11px] text-zinc-400 space-y-1.5 list-none">
                        <li className="flex items-center gap-2">
                          <div className="w-1 h-1 rounded-full bg-emerald-500/50" />
                          Используйте <span className="text-emerald-400 font-bold">@username</span> для входа
                        </li>
                        <li className="flex items-center gap-2">
                          <div className="w-1 h-1 rounded-full bg-emerald-500/50" />
                    ...
                    <div className="relative group">
                      <input
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-emerald-500/50 focus:bg-white/10 transition-all placeholder:text-zinc-600"
                        required
                      />
                    </div>
                  </motion.div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  placeholder={isRegistering ? "username" : "@username"}
                  className="w-full bg-white/5 border border-white/10 rounded-2xl pl-12 pr-5 py-4 text-white focus:outline-none focus:border-emerald-500/50 focus:bg-white/10 transition-all placeholder:text-zinc-600"
                  required
                />
                {!isRegistering && !username.startsWith('@') && username.length > 0 && (
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-emerald-500/50 font-bold uppercase tracking-tighter">
                    Добавьте @
                  </div>
                )}
              </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              type="submit"
              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-emerald-900/20 flex items-center justify-center gap-2"
            >
            ...
            <button
              ...
              className="w-full text-zinc-500 hover:text-white text-sm transition-colors font-medium"
            >
            ...
            {showSettings ? (
                          ...
                          <div className="grid grid-cols-1 gap-8">
                            <section className="space-y-2">
                              ...
                              {[
                                ...
                                { id: 'appearance', icon: Palette, label: userSettings?.language === 'Russian' ? 'Оформление' : 'Appearance', color: 'text-pink-400', bg: 'bg-pink-400/10' },
                                ...
                                <div className="pt-4 border-t border-white/10 space-y-3">
                                  <div className="flex items-center justify-between mb-2">
                                    <span className="text-xs text-zinc-500">
                                      {userSettings?.language === 'Russian' ? 'Статус разрешений:' : 'Permission Status:'}
                                    </span>
                                    <span className={cn(
                                      "text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full",
                                      !('Notification' in window) ? "bg-red-500/10 text-red-500" :
                                      ...
                                      {!('Notification' in window) ? (userSettings?.language === 'Russian' ? 'Не поддерживается' : 'Not Supported') :
                                       Notification.permission === 'granted' ? (userSettings?.language === 'Russian' ? 'Разрешено' : 'Granted') :
                                       Notification.permission === 'denied' ? (userSettings?.language === 'Russian' ? 'Заблокировано' : 'Denied') :
                                       (userSettings?.language === 'Russian' ? 'Не запрошено' : 'Default')}
                                    </span>
                                  </div>
                                  <button 
                                    onClick={() => requestNotificationPermission()}
                                    disabled={!('Notification' in window) || Notification.permission === 'denied'}
                                    className="w-full py-3 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 text-sm font-bold rounded-2xl border border-emerald-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                  >
                                    <Bell className="w-4 h-4" />
                                    {userSettings?.language === 'Russian' ? 'Запросить разрешение' : 'Request Permission'}
                                  </button>
                                  {Notification.permission === 'granted' && (
                                    <button 
                                      onClick={() => testNotification()}
                                      className="w-full py-3 bg-white/5 hover:bg-white/10 text-zinc-300 text-sm font-bold rounded-2xl border border-white/10 transition-all flex items-center justify-center gap-2"
                                    >
                                      <Monitor className="w-4 h-4" />
                                      {userSettings?.language === 'Russian' ? 'Проверить уведомление' : 'Test Notification'}
                                    </button>
                                  )}
                                  <p className="text-[10px] text-zinc-500 text-center">
                                    {userSettings?.language === 'Russian' 
                                      ? 'Нажмите, чтобы браузер запросил разрешение на показ уведомлений. Уведомления приходят, когда вкладка не активна.' 
                                      : 'Click to prompt the browser for permission. Notifications appear when the tab is not active.'}
                                  </p>
                                </div>
                              </div>
                          ...
                          {activeSettingsTab === 'appearance' && (
                            <div className="space-y-8">
                              <div className="bg-white/5 rounded-3xl p-8 space-y-6">
                                <h4 className="text-lg font-bold text-white">{userSettings?.language === 'Russian' ? 'Обои чата' : 'Chat Wallpaper'}</h4>
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                  {[
                                    { id: 'default', name: 'Default', color: 'bg-[#0d0d0d]' },
                                    { id: 'emerald', name: 'Emerald', color: 'bg-emerald-900/20' },
                                    { id: 'indigo', name: 'Indigo', color: 'bg-indigo-900/20' },
                                    { id: 'rose', name: 'Rose', color: 'bg-rose-900/20' },
                                    { id: 'amber', name: 'Amber', color: 'bg-amber-900/20' },
                                    { id: 'doodle', name: 'Doodle', color: 'bg-[url("https://www.transparenttextures.com/patterns/carbon-fibre.png")] bg-zinc-900' },
                                    { id: 'stars', name: 'Stars', color: 'bg-[url("https://www.transparenttextures.com/patterns/stardust.png")] bg-zinc-900' },
                                    { id: 'circuit', name: 'Circuit', color: 'bg-[url("https://www.transparenttextures.com/patterns/circuit-board.png")] bg-zinc-900' },
                                    { id: 'dark-matter', name: 'Dark Matter', color: 'bg-gradient-to-br from-zinc-900 via-black to-zinc-900' },
                                  ].map((wp) => (
                                    <button
                                      key={wp.id}
                                      onClick={() => updateSetting('wallpaper', wp.id)}
                                      className={cn(
                                        "aspect-[4/5] rounded-2xl border-2 transition-all relative overflow-hidden group",
                                        userSettings?.wallpaper === wp.id ? "border-emerald-500 scale-105 shadow-lg shadow-emerald-500/20" : "border-white/5 hover:border-white/20"
                                      )}
                                    >
                                      <div className={cn("absolute inset-0", wp.color)} />
                              ...
                              <div className="bg-white/5 rounded-3xl p-8 space-y-6">
                                <h4 className="text-lg font-bold text-white">{userSettings?.language === 'Russian' ? 'Темы оформления' : 'Color Themes'}</h4>
                                  ...
                                  {['emerald', 'blue', 'purple', 'rose', 'amber'].map((color) => (
                                    <button
                                      ...
                                      className={cn(
                                        ...
                                        color === 'emerald' ? "bg-emerald-500 border-emerald-500/20" :
                                        color === 'blue' ? "bg-blue-500 border-blue-500/20" :
                                        color === 'purple' ? "bg-purple-500 border-purple-500/20" :
                                        color === 'rose' ? "bg-rose-500 border-rose-500/20" :
                                        "bg-amber-500 border-amber-500/20",
                                        color === 'emerald' && "ring-2 ring-emerald-500 ring-offset-4 ring-offset-[#0d0d0d]"
                                      )}
                                    />
                                  ))}
                                </div>
                                <p className="text-xs text-zinc-500 italic">Custom accent colors coming soon in next update!</p>
                              </div>
                            </div>
                          )}
              ...
              >
                ...
                <div className="flex-1 relative overflow-hidden">
                  {/* Chat Wallpaper */}
                  <div className={cn(
                    "absolute inset-0 pointer-events-none opacity-40 transition-all duration-700",
                    userSettings?.wallpaper === 'emerald' ? "bg-emerald-900/20" :
                    userSettings?.wallpaper === 'indigo' ? "bg-indigo-900/20" :
                    userSettings?.wallpaper === 'rose' ? "bg-rose-900/20" :
                    userSettings?.wallpaper === 'amber' ? "bg-amber-900/20" :
                    userSettings?.wallpaper === 'doodle' ? "bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] bg-zinc-900" :
                    userSettings?.wallpaper === 'stars' ? "bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] bg-zinc-900" :
                    userSettings?.wallpaper === 'circuit' ? "bg-[url('https://www.transparenttextures.com/patterns/circuit-board.png')] bg-zinc-900" :
                    userSettings?.wallpaper === 'dark-matter' ? "bg-gradient-to-br from-zinc-900 via-black to-zinc-900" :
                    "bg-transparent"
                  )} />
                  
                  <div 
                    ref={scrollContainerRef}
                    className="absolute inset-0 overflow-y-auto p-8 space-y-6 scroll-smooth custom-scrollbar"
                  >
                      <LayoutGroup>
                      ...
                      <div className="relative">
                        <div className={cn(
                          ...
                          (msg.mediaType === 'video' || msg.mediaType === 'poll' || msg.mediaType === 'sticker') ? "p-0 bg-transparent border-none shadow-none hover:bg-transparent" : ""
                        )}>
                          ...
                          {(!msg.mediaUrl || (msg.content && !msg.content.startsWith('[') && !msg.content.endsWith(']'))) && msg.mediaType !== 'poll' && msg.mediaType !== 'sticker' && (
                        ...
                        {(msg.sender === username || userList.find(u => u.id === username)?.role === 'admin') && (
                          <button
                            onClick={() => msg.id && deleteMessage(msg.id)}
                            className={cn(
                              "absolute top-1/2 -translate-y-1/2 p-2 rounded-full transition-all opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-400",
                              msg.sender === username ? "-left-20" : "-right-20"
                            )}
                          >
              ...
              <div className="absolute inset-0 z-0">
                <video
                  ...
                  className={cn(
                    ...
                    activeCall.remoteStream && activeCall.remoteStream.getVideoTracks().length > 0 ? "opacity-60" : "opacity-0 pointer-events-none"
                  )}
                />
                <audio
                  autoPlay
                ...
                <div className="absolute top-4 left-4 z-50">
                   <button 
                     ...
                     className="bg-white/10 hover:bg-white/20 backdrop-blur-md text-white text-[10px] px-3 py-1 rounded-full border border-white/10"
                   >
                     {userSettings?.language === 'Russian' ? 'Исправить звук' : 'Fix Audio'}
                   </button>
                </div>
            ...
            >
              ...
              >
                <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/5">
                  ...
                  <button onClick={() => setShowPollModal(false)} className="p-2 hover:bg-white/10 rounded-xl transition-colors">
                    <X className="w-5 h-5 text-zinc-400" />
                  </button>
                </div>
                <div className="p-8 space-y-6">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest ml-1">Вопрос</label>
                    <input
                      type="text"
                      value={pollQuestion}
                  ...
                  <div className="space-y-3">
                    <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest ml-1">Варианты</label>
                    {pollOptions.map((option, index) => (
                      <div key={index} className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-emerald-500/50 transition-all"
                        />
                        {pollOptions.length > 2 && (
                          <button 
                            onClick={() => setPollOptions(pollOptions.filter((_, i) => i !== index))}
                            className="p-4 text-zinc-500 hover:text-red-400 transition-colors"
                          >
                            <X className="w-5 h-5" />
                          </button>
                        )}
                    ...
                    {pollOptions.length < 6 && (
                      <button 
                        ...
                        className="w-full py-3 border border-dashed border-white/10 rounded-2xl text-zinc-500 hover:text-emerald-500 hover:border-emerald-500/50 transition-all flex items-center justify-center gap-2 text-sm font-medium"
                      >
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:hover:bg-emerald-600 rounded-2xl text-white font-bold shadow-lg shadow-emerald-900/20 transition-all active:scale-[0.98] mt-4"
                  >
                    Создать опрос
                  </button>
                </div>
