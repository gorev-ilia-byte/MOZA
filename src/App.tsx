...
export default function App() {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      socket.on('user-list', (users: { id: string; name: string }[]) => {
        setUserList(users.filter(u => u.name !== username));
      });

      socket.on('group-list', (groups: Room[]) => {
        setGroupList(groups);
      });

      socket.on('new-group', (group: Room) => {
        setGroupList(prev => [...prev, group]);
      });

      socket.on('profile-data', (data: Profile) => {
        setProfile(data);
        setTempBio(data.bio);
      });

      socket.on('profile-updated', (data: Profile) => {
        setProfile(data);
        setIsEditingBio(false);
      });

      socket.on('settings-data', (data: UserSettings) => {
        setUserSettings(data);
      });

      socket.on('muted-chats-data', (data: string[]) => {
        setMutedChats(data);
      });

      socket.on('blocked-users-data', (data: string[]) => {
        setBlockedUsers(data);
      });

      socket.on('chat-cleared', (room: string) => {
      ...
      socket.on('premium-key-generated', (data: { key: string }) => {
        setGeneratedPremiumKey(data.key);
        setPremiumMessage('Key generated successfully!');
      });

      socket.on('premium-success', (data: { message: string; expiresAt: string }) => {
        setPremiumMessage(data.message);
        setIsPremium(true);
        setPremiumExpiresAt(data.expiresAt);
      });

      socket.on('premium-error', (msg: string) => {
        setPremiumMessage(msg);
      });

      socket.on('search-results', (results: { id?: string; name: string; bio?: string; avatar_url?: string; isOnline?: boolean }[]) => {
        setSearchResults(results);
      });

      socket.on('stories-updated', (updatedStories: any[]) => {
        setStories(updatedStories);
      });

      socket.on('recent-chats', (chats: any[]) => {
      ...
      socket.on('new-message', (msg: Message) => {
        ...
        if (msg.room === activeRoom) {
          ...
          socket.emit('mark-read', { room: activeRoom, username });
        } else {
          setUnreadCounts(prev => ({
            ...prev,
        ...
        if (msg.sender !== username && !mutedChats.includes(msg.room)) {
          playMessageSound();
        }
        socket.emit('get-recent-chats', username);
      });

      socket.on('message-updated', (data: { id: number; isFavorite: boolean }) => {
      ...
      socket.on('favorites-list', (favorites: Message[]) => {
        setFavoriteMessages(favorites);
      });

      socket.on('group-updated', (group: Room) => {
        setGroupList(prev => prev.map(g => g.id === group.id ? group : g));
      });

      socket.on('group-deleted', (roomId: string) => {
      ...
      socket.on('password-verified', (data: { roomId: string; success: boolean }) => {
        if (data.success) {
          ...
          if (room) {
            ...
            socket.emit('join-room', { room: room.id, username });
          }
        } else {
      ...
      socket.on('room-call-started', (data: { roomId: string; participants: string[]; type: 'audio' | 'video'; startedBy: string }) => {
      ...
      socket.on('room-call-updated', (data: { roomId: string; participants: string[] }) => {
      ...
      socket.on('user-joined-call', async (data: { roomId: string; username: string; socketId: string }) => {
        ...
        if (activeCall && activeCall.isGroup && activeCall.roomId === data.roomId) {
          ...
          const pc = setupPeerConnection(data.socketId);
          localStream.current?.getTracks().forEach(track => {
            if (localStream.current) pc.addTrack(track, localStream.current);
          });
          const offer = await pc.createOffer();
          await pc.setLocalDescription(new RTCSessionDescription(offer));
          socket.emit('call-user', { to: data.username, offer, from: username, type: activeCall.type });
        }
      });

      socket.on('user-left-call', (data: { roomId: string; username: string; socketId: string }) => {
        if (activeCall && activeCall.isGroup && activeCall.roomId === data.roomId) {
          ...
          const audioCtx = audioContexts.current.get(data.socketId);
          if (audioCtx) {
            audioCtx.close();
            audioContexts.current.delete(data.socketId);
          }
          ...
          setActiveCall(prev => {
            ...
            delete newRemoteStreams[data.socketId];
      ...
      // WebRTC Signaling
      socket.on('call-made', async (data: { offer: any; socket: string; from: string; type: 'audio' | 'video'; isGroup?: boolean }) => {
        setSocketToUsername(prev => {
          const newMap = new Map(prev);
          newMap.set(data.socket, data.from);
          return newMap;
        });
        let isAlreadyInCall = false;
        let shouldShowIncomingCall = false;
        setActiveCall(prev => {
          if (prev && !prev.isIncoming && data.offer) {
            isAlreadyInCall = true;
          }
          if (!prev) {
            shouldShowIncomingCall = true;
          }
          return prev;
        });

        if (isAlreadyInCall) {
          // We are the initiator and someone is sending us an offer to join
          const pc = setupPeerConnection(data.socket);
          localStream.current?.getTracks().forEach(track => {
            if (localStream.current) pc.addTrack(track, localStream.current);
          });
        ...
        if (shouldShowIncomingCall) {
          setActiveCall({ from: data.from, type: data.type, isIncoming: true, isGroup: data.isGroup });
      ...
      socket.on('answer-made', async (data: { socket: string; answer: any }) => {
        const pc = peerConnections.current.get(data.socket);
        if (pc) {
          await pc.setRemoteDescription(new RTCSessionDescription(data.answer));
        }
      });

      socket.on('ice-candidate', async (data: { socket: string; candidate: any }) => {
        if (data.candidate) {
          const pc = peerConnections.current.get(data.socket);
          if (pc) {
      ...
      return () => {
        ...
        socket.off('favorites-list');
        socket.off('group-updated');
        socket.off('password-verified');
        socket.off('room-call-started');
        socket.off('room-call-updated');
        ...
        socket.off('muted-chats-data');
        socket.off('blocked-users-data');
        socket.off('chat-cleared');
      };
    }
  }, [isLoggedIn, username, activeRoom, blockedUsers, mutedChats, userSettings]);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (username.trim() && password.trim()) {
      setAuthError('');
      if (isRegistering) {
        socket.emit('register', { username: username.trim(), password: password.trim() });
      } else {
        socket.emit('login', { username: username.trim(), password: password.trim() });
      }
    }
  };
  ...
  const setupPeerConnection = (targetSocketId: string) => {
    ...
    pc.ontrack = (event) => {
      ...
      if (stream.getAudioTracks().length > 0 && !audioContexts.current.has(targetSocketId)) {
        monitorAudioStream(stream, targetSocketId);
      }
      setActiveCall(prev => {
        if (!prev) return null;
        const newRemoteStreams = { ...prev.remoteStreams, [targetSocketId]: stream };
  ...
  const startCall = async (targetName: string, type: 'audio' | 'video') => {
    ...
    try {
      ...
      if (activeRoom?.startsWith('group-')) {
        ...
        const target = userList.find(u => u.name === targetName);
        if (target) {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
                ...
                <AnimatePresence mode="popLayout">
                  {searchResults.map((user, i) => (
                    <motion.button
                      ...
                      onClick={() => joinRoom(user.name)}
                      className={cn(
                        "w-full flex items-center gap-3 p-3 rounded-xl transition-all group",
                        activeRoom?.includes(user.name) ? "bg-emerald-500/10 text-emerald-500" : "hover:bg-white/5 text-zinc-400 hover:text-white"
                      )}
                    >
                      ...
                      <div className="flex-1 text-left">
                        ...
                        {user.bio && <div className="text-[10px] text-zinc-500 truncate max-w-[150px]">{user.bio}</div>}
                      </div>
                    </motion.button>
                  ))}
                </AnimatePresence>
                  ...
                  {recentChats.map((chat, i) => (
                    ...
                    >
                      <div className="relative">
                        ...
                        {userList.some(u => u.name === chat.username) && (
          ...
          {searchQuery.trim() === '' && (
            <div>
              ...
              <div className="space-y-1">
                <AnimatePresence mode="popLayout">
                  {userList.filter(u => u.name !== username && !recentChats.some(chat => chat.username === u.name)).map((user, i) => (
                    <motion.button
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
                      exit={{ opacity: 0, scale: 0.95 }}
          ...
          <div>
            ...
            <div className="p-4 border-t border-white/10 bg-black/20">
              <div className="flex items-center justify-between px-2">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-emerald-600 rounded-xl flex items-center justify-center font-bold shadow-lg shadow-emerald-900/20">
                  ...
                  >
                    ...
                    >
                      {showFavorites ? (
                        ...
                        (() => {
                          const activeUser = userList.find(u => u.name === activeRoomName);
                    ...
                    <div className="flex-1 min-w-0">
                      <motion.h2 layoutId="header-title" className="font-bold text-lg tracking-tight truncate">{activeRoomName}</motion.h2>
                      <span className="text-xs text-emerald-400 font-medium uppercase tracking-widest block truncate">
                        {showFavorites ? (
                          ...
                          activeRoom?.startsWith('group-') ? t.group_chat : (userList.find(u => u.name === activeRoomName)?.bio || t.online)
                        )}
                      </span>
                    </div>
                  </div>
                  ...
                  {(showFavorites ? favoriteMessages : messages).map((msg, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        {msg.sender !== username && <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">{msg.sender}</span>}
                      </div>
                      <div className="relative">
                        <div className={cn(
                    ...
                    <AnimatePresence>
                      {showGifPicker && (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: 20 }}
                          className="absolute bottom-full mb-4 left-4 lg:left-10 right-4 lg:right-10 bg-[#141414] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-80 z-50"
                        >
                          <div className="p-4 border-b border-white/10 flex gap-2">
                            <input
                              ...
                              className="flex-1 bg-black/50 border border-white/10 rounded-xl px-4 py-2 text-white text-sm focus:outline-none focus:border-emerald-500/50"
                            />
                            <button
                              onClick={() => {
                                if (gifSearchQuery === 'favorites') setGifSearchQuery('');
                                else setGifSearchQuery('favorites');
                              }}
                              className={cn("p-2 rounded-xl transition-colors", gifSearchQuery === 'favorites' ? "bg-yellow-500/20 text-yellow-500" : "text-zinc-400 hover:text-white hover:bg-white/5")}
                            >
                              <Star className="w-5 h-5" />
                            </button>
                            <button
                          ...
                          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                            {gifSearchQuery === 'favorites' ? (
                              <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                                {favoriteGifs.length === 0 ? (
                                  <div className="col-span-full text-center text-zinc-500 py-10">No favorite GIFs yet</div>
                                ) : (
                                  favoriteGifs.map((url, i) => (
                                    <div key={i} className="relative group/gif rounded-xl overflow-hidden cursor-pointer" onClick={() => sendGif(url)}>
                                      <img src={url} alt="GIF" className="w-full h-32 object-cover" referrerPolicy="no-referrer" />
                                      <button
                                        onClick={(e) => { e.stopPropagation(); toggleFavoriteGif(url); }}
                                        className="absolute top-2 right-2 p-1.5 bg-black/50 rounded-lg opacity-0 group-hover/gif:opacity-100 transition-opacity hover:bg-black/80"
                                      >
                              ...
                              <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
                                {gifs.map((gif: any) => (
                                  <div key={gif.id} className="relative group/gif rounded-xl overflow-hidden cursor-pointer" onClick={() => sendGif(gif.media_formats.gif.url)}>
                                    <img src={gif.media_formats.gif.url} alt="GIF" className="w-full h-32 object-cover" referrerPolicy="no-referrer" />
                                    <button
                                      onClick={(e) => { e.stopPropagation(); toggleFavoriteGif(gif.media_formats.gif.url); }}
                                      className="absolute top-2 right-2 p-1.5 bg-black/50 rounded-lg opacity-0 group-hover/gif:opacity-100 transition-opacity hover:bg-black/80"
                                    >
                                      <Star className={cn("w-4 h-4", favoriteGifs.includes(gif.media_formats.gif.url) ? "text-yellow-500 fill-yellow-500" : "text-white")} />
                                    </button>
                                  </div>
                                ))}
                              </div>
                    ...
                    >
                      {recordingMode ? (
                        <div className="flex-1 flex items-center justify-between px-3 lg:px-5 py-2 lg:py-3 text-sm relative">
                          {recordingMode === 'video' && (
                            <div className="absolute bottom-full right-0 mb-4 w-48 h-48 rounded-full overflow-hidden border-4 border-emerald-500/50 shadow-2xl bg-black">
                              <video 
                                ref={(el) => {
                          ...
                          <div className="flex items-center gap-3 text-emerald-500">
                            ...
                            <span className="text-zinc-400 ml-2 hidden sm:inline">
                              {recordingMode === 'audio' ? 'Recording voice message...' : 'Recording video message...'}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <button 
                    ...
                    >
                      <div className="flex-1 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="p-6 flex flex-col items-center border-b border-white/10">
                          ...
                          <h2 className="text-xl font-bold">{activeRoomName}</h2>
                          {activeRoom.startsWith('group-') ? (
                            ...
                            <p className="text-sm text-zinc-400 mt-1">
                              {userList.find(u => u.name === activeRoomName)?.bio || 'в сети'}
                            </p>
                          )}
                        </div>
                              ...
                              {(() => {
                                ...
                                return group.members.map(member => {
                                  ...
                                  if (member === username) {
                                    ...
                                    const u = userList.find(u => u.name === member);
                ...
                {activeCall.isGroup && activeRoomCall ? (
                  <div className="w-full h-full flex flex-col">
                    <div className="p-6 text-center">
                      <h2 className="text-2xl font-bold text-white mb-1">{activeCall.from}</h2>
                          ...
                          >
                            {hasVideo ? (
                              ...
                              <>
                                ...
                                <span className="text-sm font-bold text-white z-10">{participant}</span>
                            ...
                            {hasVideo && (
                              <div className="absolute bottom-3 left-3 z-10 bg-black/50 backdrop-blur-md px-2 py-1 rounded-md">
                                <span className="text-xs font-medium text-white">{participant} {participant === username && "(Вы)"}</span>
                              </div>
                            )}
                          </motion.div>
                        )})}
                  ...
                  <div className="flex flex-col items-center justify-center text-center space-y-8">
                    ...
                    <div className="space-y-3">
                      <h2 className="text-2xl font-medium text-white tracking-wide">{activeCall.from}</h2>
