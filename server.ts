...
async function ensureAdminExists() {
  try {
    ...
    // Check Firestore
    const adminDoc = await getDoc(doc(firestore, "users", adminUsername));
    if (!adminDoc.exists()) {
      await setDoc(doc(firestore, "users", adminUsername), {
...
// Migration: Sync SQLite users to Firestore
async function migrateUsersToFirestore() {
  try {
    ...
    for (const user of users) {
      ...
      if (!userDoc.exists()) {
        await setDoc(doc(firestore, "users", user.username), {
        ...
        const profile = db.prepare("SELECT * FROM profiles WHERE username = ?").get(user.username) as any;
        if (profile) {
          await setDoc(doc(firestore, "profiles", user.username), {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("register", async (data: { username: string; nickname?: string; password?: string }) => {
      ...
      try {
        ...
        if (nickname) {
          await setDoc(doc(firestore, "profiles", username), {
            username,
            nickname
          });
        }
    ...
    socket.on("generate-premium-key", async (data: { username: string }) => {
      try {
        const userDoc = await getDoc(doc(firestore, "users", data.username));
    ...
    socket.on("get-recent-chats", (username: string) => {
      ...
      const recentChats = db.prepare(`
        ...
        JOIN profiles p ON p.username = CASE 
