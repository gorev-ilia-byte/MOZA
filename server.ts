...
// Firebase Admin initialization (Disabled for now due to permission issues)
const firestore = {
  collection: function(..._args: any[]) {
    const chain: any = {
      doc: (..._args: any[]) => ({
      ...
      add: async (..._args: any[]) => ({ id: 'mock-' + Date.now() + '-' + Math.random().toString(36).substring(2, 9) }),
      where: (..._args: any[]) => chain,
      orderBy: (..._args: any[]) => chain,
      limit: (..._args: any[]) => chain,
      get: async (..._args: any[]) => ({ docs: [], size: 0, forEach: (cb: any) => {} }),
      delete: async (..._args: any[]) => {}
    };
    return chain;
  }
...
try {
  const userSettingsCols = db.prepare("PRAGMA table_info(user_settings)").all() as any[];
  if (!userSettingsCols.find(c => c.name === 'wallpaper')) {
    db.prepare("ALTER TABLE user_settings ADD COLUMN wallpaper TEXT DEFAULT 'default'").run();
    console.log("Added wallpaper column to user_settings");
  }
  
  const messagesCols = db.prepare("PRAGMA table_info(messages)").all() as any[];
  if (!messagesCols.find(c => c.name === 'poll_id')) {
    db.prepare("ALTER TABLE messages ADD COLUMN poll_id TEXT").run();
    console.log("Added poll_id column to messages");
  }
  if (!messagesCols.find(c => c.name === 'firestore_id')) {
    db.prepare("ALTER TABLE messages ADD COLUMN firestore_id TEXT").run();
    console.log("Added firestore_id column to messages");
  }
} catch (err) {
  console.error("Migration (columns) error:", err);
}

// Initial stickers
const initialStickers = [
  { id: 's1', url: 'https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/3o7TKMGpxS6S5F8f0Q/giphy.gif', pack: 'Classic' },
  { id: 's2', url: 'https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/l41lTfuxX5Wp99v5S/giphy.gif', pack: 'Classic' },
  { id: 's3', url: 'https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/3o7TKVUn7iM8FMEU24/giphy.gif', pack: 'Classic' },
  { id: 's4', url: 'https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/3o7TKMGpxS6S5F8f0Q/giphy.gif', pack: 'Classic' },
  { id: 's5', url: 'https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExNHJ6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6eXp6JmVwPXYxX2ludGVybmFsX2dpZl9ieV9pZCZjdD1z/l41lTfuxX5Wp99v5S/giphy.gif', pack: 'Classic' },
];

initialStickers.forEach(s => {
  db.prepare("INSERT OR IGNORE INTO stickers (id, url, pack_name, creator) VALUES (?, ?, ?, ?)").run(s.id, s.url, s.pack, 'system');
});

console.log("Database initialized with tables and initial stickers.");
...
async function syncFromFirestore() {
  try {
    console.log("Starting Firestore -> SQLite synchronization...");
    
    // Sync Users
    const usersSnapshot = await firestore.collection("users").get();
    console.log(`Found ${usersSnapshot.size} users in Firestore.`);
    const userStmt = db.prepare("INSERT OR REPLACE INTO users (username, password, role, is_premium, premium_expires_at, last_seen) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    console.log(`Found ${profilesSnapshot.size} profiles in Firestore.`);
    const profileStmt = db.prepare("INSERT OR REPLACE INTO profiles (username, nickname, bio, avatar_url) VALUES (?, ?, ?, ?)");
    ...
    const roomStmt = db.prepare("INSERT OR REPLACE INTO rooms (id, name, handle, type, password, is_private, owner) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    console.log(`Found ${messagesSnapshot.size} messages in Firestore.`);
    const msgStmt = db.prepare("INSERT OR REPLACE INTO messages (firestore_id, room, sender, content, timestamp, media_url, media_type) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    console.log("Sync from Firestore completed successfully.");
  } catch (err) {
    console.error("CRITICAL: Sync from Firestore failed:", err);
  }
}
...
async function startServer() {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("create-poll", (data: { room: string; question: string; options: string[]; creator: string }) => {
      const { room, question, options, creator } = data;
      const id = Math.random().toString(36).substring(2, 15);
      const optionsJson = JSON.stringify(options);
      
      db.prepare("INSERT INTO polls (id, room, question, options, creator) VALUES (?, ?, ?, ?, ?)").run(id, room, question, optionsJson, creator);
      
      const poll = { id, room, question, options, votes: {}, creator, timestamp: new Date().toISOString(), isClosed: false };
      io.to(room).emit("poll-created", poll);
      
      // Also send as a special message
      const message = {
        room,
        sender: creator,
      ...
      db.prepare("INSERT INTO messages (room, sender, content, timestamp, media_type, poll_id) VALUES (?, ?, ?, ?, ?, ?)").run(
        message.room, message.sender, message.content, message.timestamp, message.mediaType, message.pollId
      );
      io.to(room).emit("new-message", message);
    });

    socket.on("vote-poll", (data: { pollId: string; username: string; optionIndex: number }) => {
      const { pollId, username, optionIndex } = data;
      db.prepare("INSERT OR REPLACE INTO poll_votes (poll_id, username, option_index) VALUES (?, ?, ?)").run(pollId, username, optionIndex);
      
      // Get updated votes
      const votes = db.prepare("SELECT username, option_index FROM poll_votes WHERE poll_id = ?").all(pollId) as any[];
      const votesMap: { [key: number]: string[] } = {};
      votes.forEach(v => {
        if (!votesMap[v.option_index]) votesMap[v.option_index] = [];
        votesMap[v.option_index].push(v.username);
      });
      
      const poll = db.prepare("SELECT * FROM polls WHERE id = ?").get(pollId) as any;
      if (poll) {
        io.to(poll.room).emit("poll-updated", { pollId, votes: votesMap });
      }
    });

    socket.on("get-polls", (room: string) => {
      const polls = db.prepare("SELECT * FROM polls WHERE room = ?").all(room) as any[];
      const results = polls.map(p => {
        const votes = db.prepare("SELECT username, option_index FROM poll_votes WHERE poll_id = ?").all(p.id) as any[];
        const votesMap: { [key: number]: string[] } = {};
        votes.forEach(v => {
          if (!votesMap[v.option_index]) votesMap[v.option_index] = [];
          votesMap[v.option_index].push(v.username);
      ...
      socket.emit("polls-list", results);
    });

    socket.on("create-sticker", (data: { url: string; packName: string; creator: string }) => {
      const id = Math.random().toString(36).substring(2, 15);
      db.prepare("INSERT INTO stickers (id, url, pack_name, creator) VALUES (?, ?, ?, ?)").run(id, data.url, data.packName, data.creator);
      socket.emit("sticker-created", { id, ...data });
    });

    socket.on("get-stickers", () => {
      const stickers = db.prepare("SELECT * FROM stickers").all() as any[];
      socket.emit("stickers-list", stickers.map(s => ({
        id: s.id,
        url: s.url,
    ...
    socket.on("get-recent-chats", (username: string) => {
      ...
      const recentChats = db.prepare(`
        ...
        SELECT lm.room, lm.sender, lm.content, lm.timestamp,
               p.avatar_url, p.nickname, p.bio, u.is_premium,
    ...
    socket.on("delete-message", async (data: { messageId: string | number; room: string; sender: string }) => {
      try {
        const user = users.get(socket.id);
        const dbUser = db.prepare("SELECT role FROM users WHERE username = ?").get(user) as any;
        
        // Only sender or admin can delete
        if (user === data.sender || (dbUser && dbUser.role === 'admin')) {
          // Find the message first to get its info for Firestore deletion
          const msg = db.prepare("SELECT * FROM messages WHERE firestore_id = ? OR id = ?").get(data.messageId, data.messageId) as any;
          
          if (msg) {
            // Delete from local DB
            db.prepare("DELETE FROM messages WHERE firestore_id = ? OR id = ?").run(data.messageId, data.messageId);
            ...
            if (msg.firestore_id) {
              await firestore.collection("messages").doc(msg.firestore_id).delete();
            } else {
              // Fallback: search by content/sender/room
    ...
    socket.on("send-message", async (data: { room: string; sender: string; content: string; mediaUrl?: string; mediaType?: string }) => {
      ...
      try {
        ...
        // Save to local SQLite
        db.prepare("INSERT INTO messages (firestore_id, room, sender, content, timestamp, media_url, media_type) VALUES (?, ?, ?, ?, ?, ?, ?)")
          .run(docRef.id, room, sender, filteredContent, timestamp, mediaUrl || null, mediaType || null);
  ...
  httpServer.listen(PORT, "0.0.0.0", () => {
    ...
    setInterval(() => {
      try {
        ...
        console.log(`[DB HEALTH] Users: ${userCount?.count || 0}, Messages: ${msgCount?.count || 0}`);
      } catch (err) {
