  ...
  io.on("connection", (socket) => {
    ...
    socket.on("generate-premium-key", (data: { username: string }) => {
      ...
      if (user && user.role === 'admin') {
        ...
        socket.emit("premium-error", "Unauthorized");
      }
    });
    ...
    socket.on("join-network", (username: string) => {
      ...
      // Persist user
      db.prepare("INSERT OR REPLACE INTO users (username, last_seen) VALUES (?, CURRENT_TIMESTAMP)").run(username);
      
      const userList = Array.from(users.entries()).map(([id, name]) => {
        const profile = db.prepare("SELECT bio, avatar_url FROM profiles WHERE username = ?").get(name);
        return { 
          id, 
          name, 
          bio: profile?.bio || '', 
      ...
      try {
        ...
        if (data.code === 'PREMIUM2026') {
          ...
          const enrichedUsers = usersList.map(u => {
            ...
            return { id: u.username, name: u.username, bio: p?.bio, avatar_url: p?.avatar_url, is_premium: u.is_premium, role: u.role };
          });
    ...
    socket.on("update-profile", (data: { username: string; bio?: string; avatar_url?: string }) => {
      const existing = db.prepare("SELECT * FROM profiles WHERE username = ?").get(data.username);
      if (existing) {
        db.prepare("UPDATE profiles SET bio = COALESCE(?, bio), avatar_url = COALESCE(?, avatar_url) WHERE username = ?")
          .run(data.bio, data.avatar_url, data.username);
      } else {
        db.prepare("INSERT INTO profiles (username, bio, avatar_url) VALUES (?, ?, ?)")
          .run(data.username, data.bio || '', data.avatar_url || '');
      }
    ...
    socket.on("get-profile", (username: string) => {
      ...
      if (!profile) {
        profile = { username, bio: 'Digital architect and P2P enthusiast.', avatar_url: '' };
      }
